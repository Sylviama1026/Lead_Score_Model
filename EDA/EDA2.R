...
df %>% group_by(Conversion) %>%
  summarise(n = n(),
            HA_rating=mean(HA_Avg_Rating,na.rm=T)) %>%
  mutate(p=round(n/nrow(df),2))
...
df %>% group_by(Conversion) %>% 
  summarise(n = n(), 
            c_high=sum(ifelse(State_G=='High',T,F)), 
            c_mid=sum(ifelse(State_G=='Mid',T,F)),
            c_low=sum(ifelse(State_G=='Low',T,F))) 
