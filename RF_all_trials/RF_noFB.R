...
runtest0<-function(df_RF){
  ...
  for(i in 1:30){
    ...
    pre<-confusionMatrix(pred, test$Conversion, mode = "everything", positive="TRUE")$byClass['Precision']
  ...
  list(summary(Recall),summary(ACC),summary(Area),summary(Precision),
       table(Drop)[order(table(Drop))],table(Good)[order(table(Good))])
}
...
ggplot(as.data.frame(importance(rf.strata2_no_FB)), aes(x=reorder(rownames(as.data.frame(importance(rf.strata2_no_FB))),MeanDecreaseAccuracy), 
...
ggplot(as.data.frame(importance(rf.strata3_no_FB)), aes(x=reorder(rownames(as.data.frame(importance(rf.strata3_no_FB))),MeanDecreaseAccuracy), 
                                                        y=MeanDecreaseAccuracy,
                                                        fill=MeanDecreaseAccuracy))+ 
  geom_bar(stat="identity", position="dodge")+ coord_flip()+
  ylab("Mean Decrease Accuracy")+
